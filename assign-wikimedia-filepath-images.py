...
def fetch_category(category: str, limit: int = 80) -> list[dict]:
    ...
    while len(files) < limit:
        ...
        for page in pages.values():
            ...
            if not title or not mime.startswith("image/"):
                continue
            if title.lower().endswith(".svg"):
                continue
            file_name = title.removeprefix("File:")
            special = "https://commons.wikimedia.org/wiki/Special:FilePath/" + urllib.parse.quote(file_name.replace(" ", "_"), safe="(),._-") + "?width=1200"
            files.append({
                "title": title,
                "src": special,
...
for group, images in [(peru_files, peru_images), (vietnam_files, vietnam_images)]:
    for file, image in zip(group, images):
        ...
        text = file.read_text()
        text = replace_schema_image(replace_first_image(text, image["src"], alt), image["src"])
...
for page_name, image, alt in [
    ("peru-bus-tour-research-library.html", peru_images[-1], "Peru bus and tour research library Wikimedia Commons travel photo"),
    ("vietnam-research-library.html", vietnam_images[-1], "Vietnam research library Wikimedia Commons travel photo"),
]:
    page = ROOT / page_name
    text = page.read_text()
    text = re.sub(
        r'(<section class="article-hero">[\s\S]*?<img[\s\S]*?src=")[^"]+("[\s\S]*?alt=")[^"]+(")',
        rf'\1{image["src"]}\2{alt}\3',
        text,
        count=1,
    )
    page.write_text(text)
...
print(f"Assigned {len(assignments)} Wikimedia Commons FilePath images: {len(peru_files)} Peru, {len(vietnam_files)} Vietnam.")
